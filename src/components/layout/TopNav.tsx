...
import { Search, ChevronDown, Bell, Command } from "lucide-react";
import { useState } from "react";
import { useWorkspace } from "../../store/workspace";
import clsx from "clsx";

export function TopNav() {
  const { projects, activeProjectId, setActiveProject, setCommandPaletteOpen } = useWorkspace();
    ...
    <header className="flex h-14 shrink-0 items-center justify-between border-b border-border bg-panel/50 px-4">
      <div className="relative">
        ...
        {open && (
          <motion.div
            ...
            className="glass absolute left-0 top-11 z-30 w-72 rounded-xl border border-border p-1.5 shadow-2xl"
          >
            {projects.map((p) => (
              <button
                key={p.id}
                ...
                className={clsx(
                  "flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-sm transition-colors duration-150 hover:bg-elevated/70",
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          <span className="ml-3 flex items-center gap-0.5 rounded border border-border bg-panel px-1.5 py-0.5 font-mono text-[10px] text-text-tertiary">
            <Command size={10} />K
          </span>
        </button>
        ...
        <div className="ml-1 h-7 w-7 rounded-full bg-gradient-to-br from-accent to-accent-2" />
      </div>
    </header>
  );
}
