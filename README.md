...
- Database Designer — tables/columns editor, live SQL + Prisma preview, real file download
- API Designer — endpoints, methods, params, live OpenAPI preview, real file download
- System Architecture — interactive node-graph canvas (React Flow)
- Flow Designer — sequence user/system flows on a canvas
...
## What's intentionally NOT included (would require a paid backend)
No accounts, no cloud sync, no multi-user — this is a local, single-user tool
by design, so there's nothing that can expire, get paused, or cost money to run.

## Contact
Built for sale — reach out on Telegram: https://t.me/SF8_9
